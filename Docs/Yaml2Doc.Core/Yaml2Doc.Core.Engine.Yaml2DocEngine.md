...
The engine: 1) Builds a [YamlDocumentContext](Yaml2Doc.Core.Parsing.YamlDocumentContext.md) from the raw YAML. 2) Resolves a dialect via [Yaml2DocRegistry](Yaml2Doc.Core.Dialects.Yaml2DocRegistry.md) (optionally by identifier). 3) Parses the context into a [PipelineDocument](Yaml2Doc.Core.Models.PipelineDocument.md). 4) Renders the document to Markdown using the configured [IMarkdownRenderer](Yaml2Doc.Core.Interfaces.IMarkdownRenderer.md). Instances are immutable after construction; dependencies are required and validated at initialization.
...
Initializes a new instance of the [Yaml2DocEngine](Yaml2Doc.Core.Engine.Yaml2DocEngine.md) class.
...
- [Yaml2DocParseException](Yaml2Doc.Core.Models.Yaml2DocParseException.md) — Thrown when the YAML text is empty or cannot be parsed into a valid [PipelineDocument](Yaml2Doc.Core.Models.PipelineDocument.md). Low-level [YamlException](YamlDotNet.Core.YamlException.md) instances are wrapped in this exception.
